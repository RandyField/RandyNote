...
//}
function checkLength(options) {

    if(!options || !options.id || !options.target || 
       !options.minLength || !options.maxLength || !options.tooltip) return;
    ...
    if(value && value.length < options.minLength && value.length > options.maxLength){
        $(options.target).html(options.tooltip);
    } else{
        isValid = true;
    }
